<template>
  <div>
    <div v-if="['bzoj', 'uoj', 'poj', 'hdu', 'loj'].includes(channel)">
      <v-text-field v-model="remoteIdNum" :label="$t('id')" type="number" />
    </div>
    <div v-else-if="['codeforces', 'atcoder'].includes(channel)">
      <v-text-field v-model="remoteIdStr" :label="$t('id')" />
    </div>
    <div v-else-if="channel === 'traditional'">
      <input type="file" multiple ref="files" />
      <v-btn color="primary" @click="applyTrad" v-text="$t('apply')" :loading="trad.applyLoading" />
      <span class="body-1">{{ trad.hintText }}</span>
    </div>
